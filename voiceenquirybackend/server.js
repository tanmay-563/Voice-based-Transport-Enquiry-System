...
app.get('/fetchBuses/:id', (req, res) => {
  ...
  const query = `
    SELECT * 
    FROM BusSchedule 
    NATURAL JOIN BusInfo 
    JOIN DriverDetails ON BusSchedule.DriverID = DriverDetails.DriverID 
...
app.post('/CheckRoute', async (req, res) => {
  ...
  SELECT RouteId 
  FROM BusStops 
  WHERE LOWER(IntermediateStops) IN ('${fromP.toLowerCase()}', '${toP.toLowerCase()}') 
  GROUP BY RouteId 
  HAVING COUNT(DISTINCT IntermediateStops) = 2
`;
console.log(q)
